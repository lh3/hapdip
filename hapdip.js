...
function b8_distEval(args)
{
	...
	if (getopt.ind + 2 > args.length) {
		...
		print("         -b FILE    include variants *overlapping* regions in BED FILE (confident region) [null]");
		print("         -B FILE    exclude variants *contained* in regions in BED FILE (can be multiple) [null]");
...
//		print("         -e         print FN/FP (fmt: chr, start, end, indel, FN/FP)");
	...
	if (fn_bed != null) {
		warn("Reading regions to include (confident regions)...");
		var a = read_bed(fn_bed);
		sum_NP = a[2], bed_NP = a[0];
	}
	var bed_excl = [];
	warn("Reading regions to exclude...");
	for (var i = 0; i < fn_bed_excl.length; ++i) {
	...
	function read_vcf(fn, ignore_flt, min_qual)
	{
		...
		while (file.readline(buf) >= 0) {
			...
			for (var i = 0; i < a.length; ++i) {
				...
				var start, end;
				if (x[4] > 0) { // insertion
					if (vcf_pos) start = x[3] - 1, end = x[3];
					else start = end = x[3];
				} else if (x[4] == 0) start = x[3], end = x[3] + 1; // SNP
				else start = x[3], end = x[3] + (-x[4]); // deletion
				if (!flt && bed_excl.length) {
					for (var j = 0; j < bed_excl.length; ++j) {
						var bed = bed_excl[j][t[0]], ret;
						if (bed != null && (ret = bed(start, end)) != null && ret[0] <= start && ret[1] >= end) {
							flt = true;
							break;
						}
					}
	...
	warn("Reading truth VCF "+args[getopt.ind]+"...");
	var truth = read_vcf(args[getopt.ind], true, 0);
	warn("Reading test VCF "+args[getopt.ind+1]+"...");
