...
function b8_parse_vcf1(t) // t = vcf_line.split("\t")
{
	...
	for (var i = 0; i < s.length; ++i) {
		...
		if (t[3].length == 1 && s[i].length == 1) { // SNP; this is a special case of the last "else", but is faster
			...
			var en_l, st;
			for (en_l = 0; en_l < s[i].length && en_l < t[3].length; ++en_l)
				if (s[i].charAt(s[i].length - 1 - en_l) != t[3].charAt(t[3].length - 1 - en_l))
					break;
			for (st = 0; st < s[i].length - en_l && st < t[3].length - en_l; ++st) // skip matching bases at the left hand
				if (s[i].charAt(st) != t[3].charAt(st))
					break;
...
{
	var c, fn_bed = null, fn_hp = null, max_d = 10, eval_snp = true, eval_indel = true, show_err = false, hp = null;
	while ((c = getopt(args, "b:d:ISep:")) != null) {
	...
	if (fn_hp != null) {
		...
		while (file.readline(buf) >= 0) {
			...
			hp.put(t[0] + ':' + t[1] + t[3].toUpperCase());
		}
		buf.destroy();
		file.close();
	}
				...
				if (hp != null && x[4] != 0) {
					...
					for (j = 1; j < seq.length; ++j)
						if (seq.charAt(j) != seq.charAt(0))
							break;
					if (j == seq.length) { // homopolymer indel
						var key = t[0] + ':' + x[3] + seq.charAt(0);
						if (hp.get(key) != null) continue;
					}
				}
